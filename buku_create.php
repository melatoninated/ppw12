<?php
$pageTitle = 'Tambah Buku - BookNest';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        $cek = $pdo->prepare('SELECT COUNT(*) FROM buku WHERE kode_buku = ?');
        $cek->execute([$data['kode_buku']]);
    ...
    if (empty($errors)) {
        $stmt = $pdo->prepare('INSERT INTO buku (kode_buku, judul, penulis, kategori, tahun_terbit, stok, status, lokasi_rak) VALUES (?, ?, ?, ?, ?, ?, ?, ?)');
        $stmt->execute([
            $data['kode_buku'], $data['judul'], $data['penulis'], $data['kategori'],
            $data['tahun_terbit'], $data['stok'], $data['status'], $data['lokasi_rak']
        ]);
...
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <p class="text-secondary fw-semibold mb-1">Create</p>
        <h2 class="fw-bold mb-0">Tambah Buku Baru</h2>
    </div>
...
<div class="soft-card p-4 p-lg-5">
    ...
    <form method="post" class="row g-3">
        <div class="col-md-4">
            ...
            <input type="text" name="kode_buku" class="form-control" placeholder="Contoh: BN-TEC-006" value="<?= e($data['kode_buku']); ?>">
        </div>
        <div class="col-md-8">
        ...
        <div class="col-md-3">
            ...
            <input type="text" name="lokasi_rak" class="form-control" placeholder="Contoh: A1" value="<?= e($data['lokasi_rak']); ?>">
        </div>
        <div class="col-12 pt-3">
            <button class="btn btn-forest rounded-pill px-5" type="submit">Simpan Buku</button>
        </div>
    </form>
</div>
